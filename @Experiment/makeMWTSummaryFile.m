function  makeMWTSummaryFile(expt, prefix, fileNum, locInFile, varargin)
%function  makeMWTSummaryFile(expt, prefix, fileNum, locInFile, varargin)
%creates MWT .summary files (.summary and .blob or .blobs that can be loaded into
%                   MWT choreography)
%
...
makeMultipleBlobsFile = existsAndDefault('fileNum', []) && existsAndDefault('locInFile', []);
...
%make summary file        
if (giveUpdates)
    disp([num2str(toc(ts1)), ' s: doing summary calculations']);
end
et = expt.elapsedTime;
...
for j = frameNum
    fprintf(fid, '%d ', data(1,j));
    fprintf(fid, '%8g ', data(2:end,j));
    if (any(startFrame == j) || any(endFrame == j))
        fprintf(fid, ' %%%% ');
        if (any(startFrame == j))
