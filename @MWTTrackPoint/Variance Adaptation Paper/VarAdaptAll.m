...
if(bayes)
    ...
    if(strcmpi(timeType, 'eti'))
        if(isfinite(opstruct.timeRange(2)))              %jwolk 10/31    changed from btdstruct.varops to opstruct
            tx_bayes = opstruct.timeRange(1):deltaT:opstruct.timeRange(2);          %jwolk 10/31
        else
            tx_bayes = opstruct.timeRange(1):deltaT:max(btdstruct.var(1).fullensemble.eti);          %jwolk 10/31
        end
    else
