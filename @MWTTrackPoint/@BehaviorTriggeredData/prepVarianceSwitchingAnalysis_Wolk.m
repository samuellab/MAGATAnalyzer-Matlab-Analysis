function btdstruct = prepVarianceSwitchingAnalysis_Wolk(btdstruct, opstruct)
...
for whichStim = 1:length(btdstruct.varops.stim)
    ...
    if strcmp(btdstruct.varops.stim(whichStim).gqname, 'led1ValDiff')
        ...
        kname_levels = [gqname_levels 'TurnLin'];
    ...
    gq_conv_LK = strcat(gq, 'TurnLin_var_low_vals');
    gq_conv_HK = strcat(gq, 'TurnLin_var_high_vals');
    if(any(btdstruct.btd.findField(gq_conv) <= 0)) %for the case where gq is led1Val or led2Val
        ...
        [turndataLK,~,texpnumLK] = btd.behaviorTriggeredDataMatrix('turn', 'start', gq_conv_LK, 0);
        [alldataLK,~,aexpnumLK] =  btd.behaviorTriggeredDataMatrix('all', 'start', gq_conv_LK , 0);
        [turndataHK,~,texpnumHK] = btd.behaviorTriggeredDataMatrix('turn', 'start', gq_conv_HK, 0);
        [alldataHK,~,aexpnumHK] =  btd.behaviorTriggeredDataMatrix('all', 'start', gq_conv_HK, 0);
        
    end
    if strcmp(btdstruct.varops.stim(whichStim).gqname, 'led1ValDiff') || strcmp(btdstruct.varops.stim(whichStim).gqname, 'led2ValDiff')
        [turndata_lvl,~,texpnum_lvl] = btd.behaviorTriggeredDataMatrix('turn', 'start', kname_levels, 0);
        [alldata_lvl,~,aexpnum_lvl] =  btd.behaviorTriggeredDataMatrix('all', 'start', kname_levels, 0);
    end
    
    if(isfield(opstruct, 'convkernel_ber_120'))
        [turndata_ber120,~,texpnum_ber120] = btd.behaviorTriggeredDataMatrix('turn', 'start', 'led2ValDiffTurnLin_ber120', 0);
        [alldata_ber120,~,aexpnum_ber120] =  btd.behaviorTriggeredDataMatrix('all', 'start', 'led2ValDiffTurnLin_ber120', 0);
    
        var(whichStim).fullensemble.x_conv_ber120 = alldata_ber120(allvalid);
        var(whichStim).fullensemble.eti_ber120 = alleti (allvalid);
        var(whichStim).fullensemble.expnum_ber120 = aexpnum_ber120 (allvalid);  
    
        var(whichStim).noturn.x_conv_ber120 = alldata_ber120(isrun & allvalid);
        var(whichStim).noturn.eti_ber120 = alleti (isrun & allvalid);
        var(whichStim).noturn.expnum_ber120 = aexpnum_ber120  (isrun & allvalid);
   
        var(whichStim).turn.x_conv_ber120 = turndata_ber120(turnvalid);
        var(whichStim).turn.eti_ber120 = turneti (turnvalid);
        var(whichStim).turn.expnum_ber120 = texpnum_ber120 (turnvalid);
    end
    var(whichStim).gqname =  btdstruct.varops.stim(whichStim).gqname;
    
    var(whichStim).fullensemble.x_conv = alldata(allvalid);
    var(whichStim).fullensemble.x_conv_LK = alldataLK(allvalid);
    var(whichStim).fullensemble.x_conv_HK = alldataHK(allvalid);
    ...
    var(whichStim).noturn.x_conv_LK = alldataLK(isrun & allvalid);
    var(whichStim).noturn.x_conv_HK = alldataHK(isrun & allvalid);
    ...
    var(whichStim).turn.x_conv_LK = turndataLK(turnvalid);
    var(whichStim).turn.x_conv_HK = turndataHK(turnvalid);
    var(whichStim).turn.eti = turneti (turnvalid);
    var(whichStim).turn.expnum = texpnum (turnvalid);

    if strcmp(btdstruct.varops.stim(whichStim).gqname, 'led1ValDiff') || strcmp(btdstruct.varops.stim(whichStim).gqname, 'led2ValDiff')
        var(whichStim).fullensemble.x_conv_lvl = alldata_lvl(allvalid);
        var(whichStim).fullensemble.eti_lvl = alleti (allvalid);
        var(whichStim).fullensemble.expnum_lvl = aexpnum_lvl(allvalid);  
    
        var(whichStim).noturn.x_conv_lvl = alldata_lvl(isrun & allvalid);
        var(whichStim).noturn.eti_lvl = alleti (isrun & allvalid);
        var(whichStim).noturn.expnum_lvl = aexpnum_lvl  (isrun & allvalid);
    
        var(whichStim).turn.x_conv_lvl = turndata_lvl(turnvalid);
        var(whichStim).turn.eti_lvl = turneti (turnvalid);
        var(whichStim).turn.expnum_lvl = texpnum_lvl (turnvalid);
    end
...
kname_low = [gqname 'TurnLin_var_low_vals'];
kname_high = [gqname 'TurnLin_var_high_vals'];
...
%         s = std(stimInput);
    ...
    if strcmp(opstruct.stim(j).gqname, 'led1ValDiff') || strcmp(opstruct.stim(j).gqname, 'led2ValDiff')
        [convkernel_l, btd] = btdstruct.btd.createBTAKernel('turn', 'start', gqname_levels, kernelTime, kernelDt, 'newFieldName',kname_levels,'abbott',true);
        %convkernel_l = btdstruct.btd.behaviorTriggeredAverage('turn','start', gqname_levels , taxis);
        %ker_mean_shift = mean(btdstruct.btd.behaviorTriggeredAverage('turn','start', gqname_levels , 10:0.1:15));
        %ck = convkernel_l - ker_mean_shift;
        %ck = csaps(1:length(ck), ck, kernelDt, 1:length(ck));
        %convkernel_l = ck./sqrt(sum(ck.^2));
        %btdstruct.btd = btdstruct.btd.addConvolvedFields(gqname_levels, kname_levels, convkernel_l, kernelDt, 'scaleToSqr', true);
        stimInput_l = btd.behaviorTriggeredDataMatrix ('all', '', kname_levels, 0);
        turnInput_l = btd.behaviorTriggeredDataMatrix('turn', 'start', gqname_levels, taxis);
    
        if(opstruct.stim(j).switch)
            inds_low = islow_all & stimToff > kernelTime; %guarantee only in low variance portion
            inds_high = ishigh_all & stimTon>kernelTime;
            sl_l = std(stimInput_l(inds_low));
            sh_l = std(stimInput_l(inds_high));
            s_l = sl_l;
            btdstruct.kernelScaling_l(j) = s_l;
        else
            s_l = std(stimInput_l) / opstruct.stim(j).var;
        end
    
        convkernel_l = convkernel_l / s_l;
    end
...
if(isfield(opstruct, 'convkernel_ber_120'))
    btdstruct.convkernel_ber_120{1} = opstruct.convkernel_ber_120; 
    btdstruct.btd = btdstruct.btd.addConvolvedFields(gqname, 'led2ValDiffTurnLin_ber120', btdstruct.convkernel_ber_120{1}, kernelDt, 'scaleToSqr', true);    
end

btdstruct.btd = btdstruct.btd.addConvolvedFields(gqname, kname, convkernel, kernelDt, 'scaleToSqr', true);

if strcmp(opstruct.stim(j).gqname, 'led1ValDiff') || strcmp(opstruct.stim(j).gqname, 'led2ValDiff')
    btdstruct.btd = btdstruct.btd.addConvolvedFields(gqname_levels, kname_levels, convkernel_l, kernelDt);
    btdstruct.convkernels_l{j} = convkernel_l;
end
...
btdstruct.btd = btdstruct.btd.addConvolvedFields(gqname, 'led2ValDiff_gf03', xgf03, kernelDt, 'symmetric', true);

xgf06 = normpdf(-20:1:20,0,0.6);
xgf06 = xgf06/sum(xgf06); 
btdstruct.btd = btdstruct.btd.addConvolvedFields(gqname, 'led2ValDiff_gf06', xgf06, kernelDt,'symmetric', true);

xgf1 = normpdf(-20:1:20,0,1);
xgf1 = xgf1/sum(xgf1); 
btdstruct.btd = btdstruct.btd.addConvolvedFields(gqname, 'led2ValDiff_gf1', xgf1, kernelDt, 'symmetric', true);
